...
const FREE_EMAIL_DOMAINS = ["gmail.com", "yahoo.com", "outlook.com", "hotmail.com", "icloud.com", "proton.me", "aol.com"];
...
const schema = z.object({
  email: z.string().trim().email("Email inválido").refine(val => {
    const domain = val.split("@")[1]?.toLowerCase();
    return domain ? !FREE_EMAIL_DOMAINS.includes(domain) : false;
  }, "Usa tu email corporativo (no gmail/outlook)"),
  name: z.string().min(2, "Ingresa tu nombre completo"),
  linkedin: z.string().url("URL de LinkedIn inválida").includes("linkedin.com", {
...
export const CompanyForm = ({
  ...
  const defaults: Partial<CompanyFormValues> = useMemo(() => {
    ...
    if (!Number.isNaN(t)) {
      if (t < 1000) ticket = "< $1K";else if (t < 5000) ticket = "$1-5K";else if (t < 25000) ticket = "$5-25K";else if (t < 100000) ticket = "$25-100K";else ticket = "> $100K";
    }
    return {
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                field
                      ...
                      <RadioGroup className="grid grid-cols-3 gap-3" value={field.value} onValueChange={field.onChange}>
                        {["Inbound caliente", "Outbound frío", "Mixto"].map(opt => <label key={opt} className="flex items-center gap-2 rounded-md border p-2 cursor-pointer">
